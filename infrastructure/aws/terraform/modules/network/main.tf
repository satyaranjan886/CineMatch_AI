...
resource "aws_security_group" "app" {
  name        = "${local.name}-app"
  description = "API instances — only from ALB"
  vpc_id      = aws_vpc.this.id
...
resource "aws_security_group" "worker" {
  ...
  description = "Celery workers — no public ingress"
  vpc_id      = aws_vpc.this.id
...
resource "aws_security_group" "database" {
  name        = "${local.name}-db"
  description = "Postgres from app + workers only"
  vpc_id      = aws_vpc.this.id
...
resource "aws_security_group" "redis" {
  ...
  description = "Redis from app + workers only"
  vpc_id      = aws_vpc.this.id
